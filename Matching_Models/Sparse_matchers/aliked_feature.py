#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Minimal ALIKED+LightGlue wrappers used by run_avl.py."""

import cv2
import numpy as np
import torch

from .LightGlue_main.lightglue import ALIKED, LightGlue
...
def ALIKED_LG_Init(device="cuda", max_num_keypoints=4096):
...
@torch.no_grad()
def ALIKED_LG_extract(image_bgr, extractor, device=None):
    if device is None:
...
@torch.no_grad()
def ALIKED_LG_match(feats0, feats1, matcher):
